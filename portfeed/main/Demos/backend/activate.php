...
class Activate extends Generic {
	...
	private function getKey() {
		...
		if ($stmt->rowCount() < 1) {
			$this->error = '<div class="alert alert-error">' . _('That shit just aint right') . '</div>
		...
		echo "<div class=\"alert alert-success\">"._('Welcome compadre')."</div>" ._('You can now see the default access granted to new users.')."
			 <p>"._('If you wanna come aboard holla')."no-reply@port-feed.com " . address . "</p>
			 <h5>"._('What to do now?')."</h5>
			 <p>" . sprintf(_('Go to the <a href="%s"> homepage</a>'), 'home.php') . "</p>";
	...
	private function signedIn() {
		...
		if ($stmt->rowCount() < 1) {
			...
			$this->error = '<div class="alert alert-error">'._('You gotta activate your account').'</div>
					  <h5>'._('What to do now?').'</h5>'
					 . '<p>' . _('Go check your email for the link') . '</p>'
					 . '<p>' . sprintf(_('<a href="%s">resend</a> the link?'), 'activate.php?resend=1') . '</p>';
		}
	}
